...
- [ ] **卖飞/卖后回落复盘看板** — 复盘页汇总偏早 vs 卖对，按来源/时段；强化敢按规则卖的反馈。
- [ ] **仓位「下一动作」一行** — 每票：持有/减半/清仓/继续观察 + 触发价（破减仓价、深回撤线）。
- [ ] **减半价显式展示** — 仓位表标已减价 / 破此价再清（反悔窗锚）。
- [ ] **买侧冲高假到位** — 回踩到位但分时仍贴尖 → 只试探不升可买（或延后武装）。
- [ ] **主线换防持仓护栏** — sticky 刚换：旧主题进观察卖侧；新主题短时不因非主线误砍强票。
- [ ] **竞价→开盘15分钟桥** — 竞价强+开盘弱 → 降仓/撤销昨夜试探提示。
- [ ] **执行日记一键** — 买/减/清时一键记下当时建议，清洁复盘样本。
...
- [x] **Server酱推送** — 每用户 SendKey；admin 允推送；仅买卖点；买点带主线/相位/阶段；无 Key 不推；保留 Windows toast。*(2026-09-17)*
