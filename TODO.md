...
- [x] **执行日记一键** — 买/减/清自动记当时建议；仓位页「执行日记」；`/api/exec-diary`。*(2026-09-18)*
- [x] **同代码多笔成本** — `position_lots` FIFO；加仓分批；减仓扣批并重算均价；仓位表「分批」列。*(2026-09-18)*
- [x] **持仓龙虎榜席位变坏 toast** — `risk_flags`/`seat_risk`；边沿页内提醒。*(2026-09-18)*
- [x] **移动端窄屏卖卡/仓位** — 卖卡全宽按钮；仓位卡突出下一动作/当日盈亏。*(2026-09-18)*
