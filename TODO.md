...
- [ ] **开盘卖出缓冲窗（09:30–09:45）** — 分时真价，非日线近似。设计见下「卖出开盘决策算法」。
...
目标：给开盘噪声留反应时间，又避免该砍的砍不掉。

**两轨 urgency（落在 sell 卡字段，可配置）**

| 轨 | 触发（示例） | 09:30–09:45 行为 | 09:45 定案 |
|---|---|---|---|
| **必须卖 `must`** | 触止损 / 跌停风险 / 相位恐慌禁持 / 明确清仓 urgency=stop|clear 且已破位 | **立即**提示清/减，不进入观察窗；toast 可标「开盘必卖」 | 仍 must；不因反抽自动改持有 |
| **可缓冲 `watch`** | 软减半、止盈、主线退潮轻减、非止损的 trim/take | 顶栏/卡面：**观察至 09:45**；展示开盘后分时：是否继续弱（低点下移、站不回分时均价） | 仍弱 → 维持减/清；已修复（收回开盘、站上均价且未破止损）→ 降为持有或取消软卖 |

**时钟**

- `T0=09:30` 连续竞价开始；`T1=09:45` 定案（可设置 `sell_open_watch_minutes`，默认 15）。
- 竞价段（09:15–09:30）只预告「若开盘仍弱则…」，不按集合竞价价成交假设。
...
- `must`：成交时刻起算（可含开盘后首笔）；
- `watch`：默认以 **09:45 有效决策价**（或之后首笔成交）作 exit，避免用 09:31 毛刺判卖飞。

**产品形态**

- 强提示 + 一键减/清；**不硬下单**。
- 与现有 `open_mute`（买侧静音）对称：卖侧是「开盘观察窗」，不是禁卖。
